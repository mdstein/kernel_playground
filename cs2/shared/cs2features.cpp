...
inline void cs2::features::update_settings(void)
{
	...
	case 250: // nothing
		...
		break;
	case 251: // walls no aim
		config::aimbot_button = 0;
		config::triggerbot_button = 321;
		config::aimbot_fov = 2.5f;
		config::aimbot_smooth = 4.5f;
		break;
	case 252: // aim no walls
		config::aimbot_button = 321;
		config::triggerbot_button = 320;
		config::aimbot_fov = 3.0f;
		config::aimbot_smooth = 4.0f;
...
void cs2::features::run(void)
{
	...
	b_aimbot_button = (cs2::input::is_button_down(config::aimbot_button) | (b_triggerbot_button & b_mtrig_button));


	//
	// triggerbot priorities based on weapons - VERA pasta
	//
	...
	if (config::visuals_enabled == TRUE)
